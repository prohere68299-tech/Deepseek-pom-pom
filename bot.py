...
BOT_TOKEN = "8807095106:AAEvJxZX-ip-KM9ustXKV3TYUvFg9GZXE_8"   # <-- Replace with your actual token
ADMIN_IDS = []
...
def admin_keyboard(user_id):
    ...
    kb.row(
        types.InlineKeyboardButton("📤 Export Database", callback_data="admin_export_menu"),
...
def admin_export_menu_keyboard():
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.row(
        types.InlineKeyboardButton("📤 Export as .db", callback_data="admin_export_db"),
        types.InlineKeyboardButton("📤 Export as .sql", callback_data="admin_export_sql")
    )
    kb.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel"))
    return kb
...
def handle_cb(call):
    ...
    try:
        ...
        # ===== EXPORT MENU =====
            ...
            if is_admin(user_id):
                safe_edit(call.message.chat.id, call.message.message_id,
                         ...
                         reply_markup=admin_export_menu_keyboard())
            else:
                bot.answer_callback_query(call.id, "Unauthorized!")
        
        # ===== EXPORT AS .db (SIRF PLANS TABLE) =====
        elif data == "admin_export_db":
            if is_admin(user_id):
                try:
                    # Create temporary database with only plans table
                    temp_db_path = "temp_plans_export.db"
                    temp_conn = sqlite3.connect(temp_db_path)
                    temp_c = temp_conn.cursor()
                    
                    # Create plans table
                    temp_c.execute('''CREATE TABLE IF NOT EXISTS plans (
                        plan_id INTEGER PRIMARY KEY AUTOINCREMENT,
                        name TEXT NOT NULL,
                        price REAL NOT NULL,
                        validity_days INTEGER NOT NULL,
                    ...
                    main_c.execute('SELECT plan_id, name, price, validity_days, channel_link, description, media_json, is_active, created_at FROM plans')
                    ...
                    for row in rows:
                        temp_c.execute('INSERT INTO plans (plan_id, name, price, validity_days, channel_link, description, media_json, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', row)
                    ...
                    with open(temp_db_path, 'rb') as f:
                        bot.send_document(user_id, f, caption="📤 Plans + Media exported as .db")
        ...
        # ===== EXPORT AS .sql (SIRF PLANS TABLE) =====
        elif data == "admin_export_sql":
            if is_admin(user_id):
                try:
                    conn = db.get_conn()
                    c = conn.cursor()
                    
                    # Sirf plans table ka data le
                    c.execute('SELECT plan_id, name, price, validity_days, channel_link, description, media_json, is_active, created_at FROM plans')
                    rows = c.fetchall()
                    conn.close()
                    ...
                    # Generate INSERT statements for plans table only
                    dump = "-- Plans Table Export (with media)\n"
                    dump += "PRAGMA foreign_keys=OFF;\n"
                    dump += "BEGIN TRANSACTION;\n"
                    
                    # Create table structure
                    dump += '''CREATE TABLE IF NOT EXISTS plans (
                        plan_id INTEGER PRIMARY KEY AUTOINCREMENT,
                        name TEXT NOT NULL,
                        price REAL NOT NULL,
                        validity_days INTEGER NOT NULL,
                    ...
                    # Insert data
                    for row in rows:
                        plan_id, name, price, validity_days, channel_link, description, media_json, is_active, created_at = row
                        # Escape single quotes
                        name = name.replace("'", "''")
                        description = (description or "").replace("'", "''")
                        channel_link = (channel_link or "").replace("'", "''")
                        media_json = (media_json or "[]").replace("'", "''")
                        
                        dump += f"INSERT INTO plans (plan_id, name, price, validity_days, channel_link, description, media_json, is_active, created_at) "
                        dump += f"VALUES ({plan_id}, '{name}', {price}, {validity_days}, '{channel_link}', '{description}', '{media_json}', {is_active}, '{created_at}');\n"
                    ...
                    sql_bytes.seek(0)
                    bot.send_document(user_id, sql_bytes, visible_file_name="plans_export.sql", 
                                    caption="📤 Plans + Media exported as .sql")
                except Exception as e:
                    bot.send_message(user_id, f"❌ Export failed: {e}")
            else:
                bot.answer_callback_query(call.id, "Unauthorized!")
        ...
        # ===== IMPORT DATABASE =====
            ...
            if is_admin(user_id):
                ...
                safe_edit(call.message.chat.id, call.message.message_id,
                         "📥 Send a `.db` or `.sql` file to import.\n\n• `.db` → Replace entire database\n• `.sql` → Execute SQL queries",
                         reply_markup=types.InlineKeyboardMarkup().add(
                         types.InlineKeyboardButton("🔙 Cancel", callback_data="admin_panel")))
    ...
    if user_id in user_data and user_data[user_id].get('import_db'):
        ...
        if file_name.endswith('.db'):
            try:
                ...
                global WELCOME_IMAGE, WELCOME_VIDEO, WELCOME_TEXT, BOT_NAME, UPI_ID, QR_CODE
                WELCOME_IMAGE = db.get_setting('welcome_image')
                WELCOME_VIDEO = db.get_setting('welcome_video')
                WELCOME_TEXT = db.get_setting('welcome_text')
                BOT_NAME = db.get_setting('bot_name')
                UPI_ID = db.get_setting('upi_id')
                QR_CODE = db.get_setting('qr_code')
                bot.reply_to(msg, "✅ Database replaced successfully from `.db` file!")
                del user_data[user_id]
            except Exception as e:
                bot.reply_to(msg, f"❌ Failed to import .db: {e}")
                del user_data[user_id]
            return
        
        # ===== .sql FILE: EXECUTE QUERIES =====
            ...
            try:
                ...
                QR_CODE = db.get_setting('qr_code')
                if errors:
                    bot.reply_to(msg, f"⚠️ SQL imported with {executed} successful, {len(errors)} errors (ignored):\n" + "\n".join(errors[:5]))
                else:
                    bot.reply_to(msg, f"✅ SQL imported successfully! {executed} queries executed.")
                del user_data[user_id]
            except Exception as e:
                bot.reply_to(msg, f"❌ SQL import failed: {e}")
                del user_data[user_id]
            return
        
        else:
            bot.reply_to(msg, "❌ Please send a `.db` or `.sql` file only!")
            return
